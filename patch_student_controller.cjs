...
code = code.replace(/export const createStudent = async \(req: AuthRequest, res: Response\): Promise<void> => \{[\s\S]*?res\.status\(500\)\.json\(\{ message: 'Lỗi máy chủ nội bộ' \}\);\s*\}\s*\}\s*\};/, newCreateStudent);
code = code.replace(/export const updateStudent = async \(req: AuthRequest, res: Response\): Promise<void> => \{[\s\S]*?res\.status\(500\)\.json\(\{ message: "Lỗi máy chủ nội bộ" \}\);\s*\}\s*\};/, newUpdateStudent);
