...
exports.generateStudentPersonalizedInsight = exports.generateDeterministicRecommendations = exports.buildLearningSnapshot = exports.MIN_QUESTIONS_TOPIC = exports.ATTENTION_THRESHOLD = exports.STRONG_THRESHOLD = void 0;
...
const buildLearningSnapshot = async (studentId) => {
    ...
    const calcAvg = (exams) => exams.length > 0
    ...
    topicRows.forEach(r => {
        ...
        if (total >= exports.MIN_QUESTIONS_TOPIC) {
            if (acc >= exports.STRONG_THRESHOLD)
                strengths.push(perf);
            else if (acc <= exports.ATTENTION_THRESHOLD)
                focusAreas.push(perf);
        }
    });
    // 4. Attendance
    ...
    `, [studentId]);
    const notes = sessionRes.rows.map(r => r.teacher_notes).filter((n) => Boolean(n && String(n).trim() !== ''));
...
// ========================================================
const generateDeterministicRecommendations = (snapshot) => {
    ...
    if (focusAreas.length > 0) {
        actionPlan.push(`[Ưu tiên 1 - Khắc phục lỗ hổng]: Tập trung giải lại các bài tập thuộc chuyên đề ${focusAreas.map(f => f.topic).join(', ')}, đặc biệt là phương pháp nhận diện và suy luận công thức.`);
    }
    else {
        actionPlan.push(`[Ưu tiên 1 - Khắc phục lỗ hổng]: Hiện không có chuyên đề nào dưới 50%. Hãy rà soát lại các câu sai ngẫu nhiên trong bài thi gần nhất để tránh mất điểm đáng tiếc.`);
    }
    // Priority 2: Phát huy thế mạnh
    if (strengths.length > 0) {
        actionPlan.push(`[Ưu tiên 2 - Phát huy thế mạnh]: Thử sức với các bài toán vận dụng cao ở chuyên đề ${strengths.map(s => s.topic).join(', ')} để tối ưu điểm số Phần II và Phần III.`);
    }
    else {
        actionPlan.push(`[Ưu tiên 2 - Phát huy thế mạnh]: Làm thêm ít nhất 10 bài tập ở chuyên đề quen thuộc để xây dựng chuyên đề thế mạnh đạt trên 80%.`);
    }
    // Priority 3: Kỷ luật & Chuyên cần
    if (attRate < 80) {
        actionPlan.push(`[Ưu tiên 3 - Chuyên cần]: Tỷ lệ chuyên cần đang ở mức ${attRate}%. Hãy tham gia đầy đủ các buổi học tiếp theo để tránh hổng kiến thức mới.`);
    }
    else {
        actionPlan.push(`[Ưu tiên 3 - Chuyên cần]: Duy trì chuyên cần xuất sắc (${attRate}%), tích cực tương tác và đặt câu hỏi cho giáo viên khi chưa hiểu bài.`);
    }
    // Priority 4: Kế hoạch tiếp theo
    if (upcoming.length > 0) {
        actionPlan.push(`[Ưu tiên 4 - Chuẩn bị lịch trình]: Chuẩn bị trước bài cho buổi học sắp tới: ${upcoming[0].class_name} (${upcoming[0].date}).`);
    }
    else {
        actionPlan.push(`[Ưu tiên 4 - Chuẩn bị lịch trình]: Hoàn thành đầy đủ các bài tập tự luyện và chuẩn bị sẵn sàng cho bài kiểm tra định kỳ tiếp theo.`);
    }
    return {
        summary,
...
exports.generateDeterministicRecommendations = generateDeterministicRecommendations;
// ========================================================
...
const generateStudentPersonalizedInsight = async (snapshot) => {
    ...
    const deterministicInsight = (0, exports.generateDeterministicRecommendations)(snapshot);
    ...
    try {
        const ai = new genai_1.GoogleGenAI({ apiKey, httpOptions: { timeout: 30000 } });
