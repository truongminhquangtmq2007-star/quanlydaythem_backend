...
const askAITutor = async (req, res) => {
    try {
        ...
        const submissionRes = await db_1.default.query("SELECT student_answers, answers AS detailed_results FROM exam_submissions WHERE student_id = $1 AND document_id = $2 AND status = 'COMPLETED'", [studentId, exam_id]);
        ...
        const keyRes = await db_1.default.query("SELECT exam_content FROM exam_keys WHERE document_id = $1", [exam_id]);
