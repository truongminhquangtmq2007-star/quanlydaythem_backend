...
exports.askAITutor = exports.publishExam = exports.getAllExams = exports.parseExamFromFile = exports.createExamFromText = exports.getExamKey = exports.getSubmissionDetail = exports.getMySubmissions = exports.getExamSubmissions = exports.submitExam = exports.saveDraftExam = exports.getDraftExam = exports.saveAnswerKey = void 0;
...
const submitExam = async (req, res) => {
    try {
        ...
        if (Array.isArray(student_answers)) {
            student_answers.forEach((item) => {
                const qId = String(item.question_id || item.id);
                flatAnswers[qId] = item.student_answer ?? item.answer;
                if (item.part === 'part2')
                    p2Answers[qId] = item.student_answer;
                else if (item.part === 'part3')
                    p3Answers[qId] = item.student_answer;
                else
                    p1Answers[qId] = item.student_answer;
            });
        }
...
const createExamFromText = async (req, res) => {
    ...
    try {
        ...
        const part1Key = fullExam.part1.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
        const part2Key = fullExam.part2.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
        const part3Key = fullExam.part3.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
...
const parseExamFromFile = async (req, res) => {
    try {
        ...
        if (!actual_document_id || actual_document_id === 0) {
            ...
            if (class_id) {
                ...
                if (folderCheck.rows.length > 0) {
                    ...
                    const newFolder = await db_1.default.query("INSERT INTO folders (name, category, class_id) VALUES ('Đề thi', 'EXAM', $1) RETURNING id", [class_id]);
                    folderId = newFolder.rows[0].id;
                }
            }
        ...
        try {
            ...
            part1Key = fullExam.part1.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
            part2Key = fullExam.part2.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
            part3Key = fullExam.part3.reduce((acc, q) => { acc[q.id] = q.correctAnswer; return acc; }, {});
        ...
        try {
            const allQuestions = [
                ...(fullExam.part1 || []).map((q) => ({ ...q, part_number: 1, question_type: 'MULTIPLE_CHOICE' })),
                ...(fullExam.part2 || []).map((q) => ({ ...q, part_number: 2, question_type: 'TRUE_FALSE' })),
                ...(fullExam.part3 || []).map((q) => ({ ...q, part_number: 3, question_type: 'SHORT_ANSWER' }))
            ];
...
const askAITutor = async (req, res) => {
    try {
        ...
        const keyRes = await db_1.default.query("SELECT exam_content, allow_view_answers FROM exam_keys WHERE document_id = $1", [exam_id]);
        ...
        const qData = allQuestions.find((q) => String(q.id) === String(question_id));
        if (!qData) {
        ...
        const sharedCtx = sharedList.find((g) => (g.questionIds || g.question_ids || []).includes(Number(question_id)));
        ...
        if (studentId) {
            const submissionRes = await db_1.default.query("SELECT student_answers, answers AS detailed_results FROM exam_submissions WHERE student_id = $1 AND document_id = $2 ORDER BY submitted_at DESC LIMIT 1", [studentId, exam_id]);
            if (submissionRes.rows.length > 0) {
                const submission = submissionRes.rows[0];
                const detailedResults = submission.detailed_results || [];
                const questionDetail = detailedResults.find((q) => String(q.question_id) === String(question_id));
                if (questionDetail) {
...
4. Trình bày bằng Markdown, sử dụng LaTeX cho công thức toán học (bọc trong dấu $ cho inline hoặc $$ cho block).`;
        ...
        res.status(500).json({ message: 'Lỗi AI Tutor', detail: error.message });
    }
};
