...
const getProfile360 = async (req, res) => {
    try {
        ...
        const studentRes = await db_1.default.query('SELECT * FROM students WHERE id = $1', [id]);
        ...
        const classRes = await db_1.default.query(`SELECT c.id, c.class_name, c.class_name as name, c.schedule, c.meet_link, COALESCE(e.status, 'Đang học') as member_status, COALESCE(c.class_type, 'Lớp dạy kèm') as subject, c.class_type
       FROM enrollments e 
       JOIN classes c ON e.class_id = c.id 
       WHERE e.student_id = $1 AND (e.status IS NULL OR e.status IN ('ACTIVE', 'Đang học', 'active'))`, [id]);
        // 3. Tính toán chuyên cần dạng tổng hợp canonical { total, present, late, absent, rate }
        const attendRes = await db_1.default.query('SELECT status, count(*)::int as count FROM attendance WHERE student_id = $1 GROUP BY status', [id]);
        let total = 0;
        let present = 0;
        let late = 0;
        let absent = 0;
        ...
        const scoresRes = await db_1.default.query(`SELECT es.id, es.document_id, COALESCE(d.title, 'Bài thi') as exam_title, es.total_score, es.submitted_at, es.time_taken_seconds
       ...
       ORDER BY accuracy_rate DESC, total_questions DESC`, [id]);
        if (topicsRes.rows.length > 0) {
            ...
            const jsonbRes = await db_1.default.query(`SELECT topic_performance FROM exam_submissions WHERE student_id = $1 AND status = 'COMPLETED' AND topic_performance IS NOT NULL LIMIT 20`, [id]);
...
exports.getProfile360 = getProfile360;
const updateStudent = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query('UPDATE students SET full_name = $1, phone_number = $2, parent_phone = $3, school_name = $4, school = $4, grade = $5, current_level = $6, email = $7 WHERE id = $8 RETURNING *', [full_name, phoneToUse, parent_phone, schoolToUse, grade, current_level, email || null, id]);
...
const updateStudentGoals = async (req, res) => {
    const { id } = req.params;
    const { learning_goals } = req.body;
    try {
        const result = await db_1.default.query('UPDATE students SET learning_goals = $1 WHERE id = $2 RETURNING learning_goals', [learning_goals, id]);
...
exports.updateStudentGoals = updateStudentGoals;
const resetStudentPassword = async (req, res) => {
    try {
        ...
        const result = await db_1.default.query(`UPDATE users SET password_hash = $1 WHERE student_id = $2 AND role = 'STUDENT' RETURNING id`, [passwordHash, studentId]);
...
const generateAIEvaluation = async (req, res) => {
    ...
    try {
        ...
        // 3. Get attendance
        const attendanceRes = await db_1.default.query('SELECT status, COUNT(*) as count FROM attendance WHERE student_id = $1 GROUP BY status', [id]);
...
{ "strong_points": ["..."], "weak_points": ["..."], "attention_note": "...", "action_plan": "...", "analyzed_at": "YYYY-MM-DD" }`;
        // 5. Call AI
        let aiText = await (0, geminiService_1.generateWithFallback)(prompt);
        ...
        // 6. Save to DB
        await db_1.default.query('UPDATE students SET ai_evaluation = $1 WHERE id = $2', [JSON.stringify(parsedAIResponse), id]);
...
exports.generateAIEvaluation = generateAIEvaluation;
