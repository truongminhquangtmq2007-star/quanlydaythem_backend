...
const getAssignableDocuments = async (req, res) => {
    try {
        ...
        if (user?.role === 'TEACHER') {
            const checkClass = await db_1.default.query('SELECT id FROM classes WHERE id = $1 AND teacher_id = $2', [id, user.id]);
    ...
    `;
        const result = await db_1.default.query(query, [teacherId]);
...
const assignDocumentsToClass = async (req, res) => {
    ...
    try {
        ...
        for (const docId of document_ids) {
            ...
            await client.query(`INSERT INTO assignments (class_id, document_id, title, due_at, description, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`, [id, docId, assignTitle, due_at || null, sessionDesc]);
            ...
            if (folderRes.rows.length > 0) {
                ...
                const newFolder = await client.query('INSERT INTO folders (name, category, class_id, teacher_id, created_at) VALUES ($1, $2, $3, $4, NOW()) RETURNING id', [folderName, category, id, user?.id || null]);
