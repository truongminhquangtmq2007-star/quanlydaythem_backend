...
const studentLogin = async (req, res) => {
    ...
    `;
        const canonicalResult = await db_1.default.query(canonicalQuery, [identifier]);
        let user = canonicalResult.rows[0];
        // 2. Nếu không tìm thấy qua canonical JOIN, kiểm tra users chưa liên kết (student_id IS NULL)
        if (!user) {
            const unlinkedUserRes = await db_1.default.query("SELECT id, username, password_hash, full_name, student_id, title FROM users WHERE username = $1 AND role = 'STUDENT'", [identifier]);
        ...
        if (!user) {
            const legacyStudentRes = await db_1.default.query("SELECT id as student_id, username, password as password_hash, full_name, phone_number FROM students WHERE (username = $1 OR phone_number = $1) AND password IS NOT NULL", [identifier]);
            if (legacyStudentRes.rows.length > 0) {
                ...
                const linkedUserRes = await db_1.default.query("SELECT id, username, password_hash, full_name, student_id, title FROM users WHERE student_id = $1 AND role = 'STUDENT'", [legacyStudent.student_id]);
        ...
        if (!isMatch && user.student_password) {
            isMatch = await bcryptjs_1.default.compare(password, user.student_password);
        }
        if (!isMatch) {
