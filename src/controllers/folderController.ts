...
import pool from '../db'; // Nhớ trỏ đúng đường dẫn file DB của bạn
...
export const createFolder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO folders (name, category, class_id, parent_id) 
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [name, category, class_id, pId]
    );
...
export const getDriveContents = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const folderParams: any[] = [category, class_id];
    ...
    const foldersResult = await pool.query(folderQuery, folderParams);
