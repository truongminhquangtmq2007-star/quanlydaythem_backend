...
export const buildLearningSnapshot = async (studentId: number): Promise<LearningSnapshot> => {
    ...
    const calcAvg = (exams: any[]) => exams.length > 0 ? exams.reduce((s, e) => s + Number(e.total_score), 0) / exams.length : 0;
    ...
    topicsRes.rows.forEach(r => {
        if (r.total_questions >= MIN_QUESTIONS_TOPIC) {
            const topicPerf = {
                topic: r.topic_name,
            ...
            if (topicPerf.accuracy >= STRONG_THRESHOLD) strengths.push(topicPerf);
            else if (topicPerf.accuracy <= ATTENTION_THRESHOLD) focusAreas.push(topicPerf);
        }
    });
...
export const generateStudentPersonalizedInsight = async (snapshot: LearningSnapshot): Promise<any> => {
    try {
